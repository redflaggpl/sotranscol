...
class Remesas extends CActiveRecord
{
	...
	*/
	...
	/**
	* Dibuja pdf formateado para hojas por alimentación manual
	*/
	...
	/**
	* Dibuja pdf formateado para hojas en formas continuas
	*/
	public function drawPdfWindowsFc()
	{
		$pdf = Yii::createComponent('application.extensions.tcpdf.ETcPdf', 
                            'P', 'cm', 'Letter', true, 'UTF-8');
		$pdf->SetCreator(PDF_CREATOR);
		...
		/* oficina */
		$pdf->text(4,2.9,strtoupper($this->oficina->nombre));
		$pdf->text(12,2.9,$this->formatOracionFecha());
		//$pdf->text(19,2.8,$this->cargo_oficina);
		$pdf->text(4, 3.3, strtoupper($this->remitente->nombre));
		$pdf->text(4, 3.7, strtoupper($this->destinatario));
		$pdf->text(18, 3.7, strtoupper($this->telefono));
		$pdf->text(4, 4.1, strtoupper($this->direccion));
		$pdf->text(18, 4.1, strtoupper($this->ciudad->nombre));
		...
		$pdf->text(6.5, 9.3, strtoupper($this->observaciones));

		$pdf->text(4, 9.5,$this->fletes);
		$pdf->text(4, 10, $this->valor_aforo);
		$pdf->text(4, 10.5, $this->v_u);
		$pdf->text(4, 11, strtoupper($this->vehiculos->placa));
		$pdf->text(8.5, 11, strtoupper($this->vehiculos->conductor));
