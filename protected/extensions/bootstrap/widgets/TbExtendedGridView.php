...
class TbSumOperation extends TbOperation
{
	...
	public function processValue($value)
	{
		...
		$this->total += str_replace(array(',', '.'), "", $this->extractNumber($clean));
	}
	...
	{
		echo strtr($this->template, array('{label}' => $this->label, '{value}' => $this->total === null ? '' : Yii::app()->format->format('$'. number_format($this->total, 0, ',', '.'), $this->column->type)));
	}
}
